...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_insert->execute()) { 
        echo "<script>alert('Ticket submitted successfully!'); window.location.href = 'http://localhost/psau4/admin/tickets/ticket-view.php';</script>";
    } else {
        $error_message = "Error executing SQL statement: " . $sql_insert . " - Error: " . $conn->error;
        error_log($error_message);
        echo "<script>alert('$error_message'); window.location.href = 'http://localhost/psau4/admin/tickets/create-ticket.php';</script>";
    }
